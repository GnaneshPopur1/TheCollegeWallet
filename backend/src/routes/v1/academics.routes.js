...
router.post('/terms', authenticateToken, async (req, res) => {
  try {
    const { semester_name, tuition_total, aid_applied, dining_dollars_bal } = req.body;
    ...
    const term = await prisma.academic_Term.create({
      data: {
        ...
        tuition_total,
        aid_applied,
        dining_dollars_bal: dining_dollars_bal || 0
      }
    });
...
router.patch('/terms/:termId/dining', authenticateToken, async (req, res) => {
  try {
    ...
    const updatedTerm = await prisma.academic_Term.updateMany({
      ...
      data: { dining_dollars_bal: new_balance }
    });
...
router.get('/dining', authenticateToken, async (req, res) => {
  try {
    ...
    if (!term || term.dining_dollars === null) {
    ...
    const safeDailySpend = term.dining_dollars / daysRemaining;

    res.json({
      safe_daily_spend: safeDailySpend,
      days_remaining: daysRemaining,
      dining_dollars: term.dining_dollars
    });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
